...
export const FormRunner = ({
  ...
  const currentSection = form.sections[currentSectionIndex] || form.sections[0] || { id: 'sec-1', title: 'Bagian 1' };
  ...
  const handleFileUpload = async (qId: string, file: File) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(err.error || 'Gagal mengunggah berkas ke Cloudinary.');
        return;
      }

      const data = await res.json();
      // Store object with url and original name
                ...
                >
                  ...
                  <div className="text-xs text-slate-700">
                    ...
                    <span className="font-bold">
                      {Array.isArray(userVal)
                        ...
                        : typeof userVal === 'object' && userVal?.url
                        ? userVal.name
                  ...
                  {q.quiz?.explanation && (
                    <div className="text-xs text-slate-600 bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="font-bold text-slate-800 block mb-0.5">Penjelasan:</span>
    ...
    <div className="max-w-2xl mx-auto pb-24 px-3 sm:px-4 space-y-4">
      ...
      <div className="space-y-4">
        {sectionQuestions.map((q) => {
          ...
          const isUploading = Boolean(uploadingFiles[q.id]);
          ...
          return (
            ...
            >
              ...
              <div className="text-xs sm:text-sm">
                ...
                {/* 8. File Upload with Cloudinary Storage */}
                {q.type === 'file' && (
                  <div className="p-4 border-2 border-dashed border-slate-300 rounded-xl bg-slate-50/50 flex flex-col items-center justify-center gap-2 text-center transition-colors">
                    {isUploading ? (
                      <div className="flex items-center gap-2 text-xs font-semibold text-indigo-600 py-2">
                        <Loader2 className="w-5 h-5 animate-spin" />
                        <span>Mengunggah berkas ke Cloudinary...</span>
                      </div>
                    ) : userVal?.url ? (
                      <div className="flex flex-col items-center gap-2 text-xs font-bold text-emerald-700">
                        <div className="flex items-center gap-2">
                          <FileCheck className="w-5 h-5 text-emerald-600" />
                          <span>{userVal.name || 'Berkas Terunggah'}</span>
                        </div>
                        <a
                          href={userVal.url}
                          target="_blank"
                        ...
                        >
                          <span>Buka Berkas di Cloudinary</span>
                      ...
                      <>
                        ...
                        <span className="text-xs font-semibold text-slate-700">
                          Klik untuk memilih berkas (tersimpan di Cloudinary)
                        </span>
                    ...
                    <label
                      htmlFor={`file-${q.id}`}
                      className="mt-2 px-3.5 py-1.5 bg-white border border-slate-300 rounded-lg text-xs font-semibold text-slate-700 shadow-2xs hover:bg-slate-50 cursor-pointer"
                    >
                      {userVal?.url ? 'Ganti Berkas' : 'Pilih Berkas'}
                    </label>
                  </div>
                )}
      ...
      <div className="pt-4 flex items-center justify-between gap-3">
        {currentSectionIndex > 0 ? (
          ...
          <button
            type="button"
            onClick={() => {
              if (window.confirm('Kosongkan formulir?')) {
                setAnswers({});
                setRespondentEmail('');
                setValidationErrors({});
              }
